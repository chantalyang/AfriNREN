...
$.getJSON("static/data/test_measurement_1.json", function(json1) {
	    	...
	    	if (data.prb_id == 13114){
	    		$.each(data.result, function(key, data){ //Loop through the results field 
	    	 	try{
					...
					var icon_string = "static/numbers/number_"+hop_num+".png"
...
function add_fibre_layer(gmap){
	...
	fibre_data_layer.loadGeoJson("static/data/fibre.json");
...
function add_probe_layer(gmap){
	...
	probe_layer.loadGeoJson("static/data/all_probes.json");
